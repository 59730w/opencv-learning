...
Day69 交付的是一个本地命令行程序，不是网页。它接收一段视频或一个视频目录，运行已经冻结的
Day63 感知模型、Day65 时序跟踪、Day66 导航输出合同和 Day68 遮挡恢复守卫，并产生：
...
需要可用的 `opencv-python`、`numpy`、`torch`，以及本地冻结模型：
