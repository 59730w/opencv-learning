...
void print_usage() {
    std::cerr
        << "Usage: day51_parallel_pipeline <input_dir> <output_dir> "
...
int main(int argc, char* argv[]) {
    ...
    const auto operation = day51::parse_operation(argv[4]);
    if (!operation.has_value()) {
        std::cerr << "Unknown operation: " << argv[4] << '\n';
        return 4;
    }

    const auto mode = day51::parse_execution_mode(argv[6]);
    if (!mode.has_value()) {
    ...
    try {
        const day51::ImagePipeline pipeline{
            *operation,
            *mode,
            *worker_count,
        };
        const day51::PipelineReport report =
        ...
        std::cout << "Operation: " << day51::operation_name(*operation) << '\n';
        std::cout << "Mode: " << day51::execution_mode_name(report.mode) << '\n';
        ...
        day51::print_summary(report.results);
        std::cout << "DAY51_THREADS_OK\n";
