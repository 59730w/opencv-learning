...
- [04｜农业机器人作物行视觉感知](projects/04_crop_row_perception/)：Day59合同已原位修订为多作物行、左右边界和真实行间走廊中心的图像平面任务；Day60完成受限数据实审，Day61/62冻结颜色与形态学，Day63单行第二版保留为基线，Day64完成坐标与物理测量边界。下一步在现有Day63目录内重构多行几何；完整拒识与外部泛化门禁仍为 `BLOCKED`。
...
- Day59：转入农业机器人作物行视觉感知预研；审查七个开源项目/数据候选并完成目标契约。2026-09-05在原目录将合同修订为schema v2：明确检测所有可评价可见作物行、选择相机中心左右相邻边界、以边界中点计算走廊中心，并禁止把中央作物行或图像边界误称为机器人行驶中心或真实车体安全边界；多行标签补充审计尚待Day63重构前完成。
...
- Day63：第一版整线支持搜索因方向P90 9.615°、双阈值达标率60.89%而只保留为基线；第二版冻结Day61/62，改用320维多尺度掩膜特征和Extra Trees端点回归。训练开发五折逐折通过改进门，选中 `extra_depth12_leaf2`，五折平均有效率0.9264、位置MAE 0.0205、方向MAE 1.944°、双阈值达标率88.24%。在已参与开发的248张同源图上为0.9476、0.0239、1.809°和82.66%，全部开发门通过；仍无未触碰确认，个别错行、内部基准、冻结外部和拒识能力继续未验证。
