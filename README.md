...
- [04｜农业机器人作物行视觉感知](projects/04_crop_row_perception/)：Day59合同已原位修订为多作物行、左右边界和行间走廊中心任务；Day60完成多行标签实审，Day61/62冻结全幅颜色与形态学输入，Day63已在原目录完成多行重学，两个开发分区的标签排除OOF多行几何门均通过。Day64坐标定义保持有效；下一步进入Day65多行时序与安全拒识，完整拒识和外部泛化仍为 `BLOCKED`。
...
- Day64：冻结Day63第二版，在248张已复用开发图上定义图像中心偏移、归一化方向代理和单线外推点；方向复算与Day63最大差异为0°，状态原样传播。多线合成测试验证消失点数学，相机内参与地面变换合成测试验证接口；真实输出因只有单线、没有相机标定和地面变换而分别阻塞消失点、相机射线和米制测量，不宣称真实航向或机器人控制能力。
